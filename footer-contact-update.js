document.addEventListener('DOMContentLoaded', function() {
    // Update all footer contact sections with the correct information
    const footerContacts = document.querySelectorAll('.footer-contact');
    
    footerContacts.forEach(contactSection => {
        // Get existing heading
        const heading = contactSection.querySelector('h3');
        
        // Clear existing content after heading
        while (contactSection.childNodes.length > 0) {
            contactSection.removeChild(contactSection.childNodes[0]);
        }
        ...
        emailP.textContent = 'artesalaser@gmail.com';
        contactSection.appendChild(emailP);
        
        const phoneP = document.createElement('p');
        phoneP.textContent = '690 63 97 12';
        contactSection.appendChild(phoneP);
        
        // Add only Instagram icon
